...
| ` (backtick) | Toggle developer console (pauses game while open) |
...
| 13 | ✅ Done | Power shot 15 shots + hit-freeze + recoil + bassier SFX, autofire powerup (50 rapid shots with spread), muzzle flash, final-symbol fanfare SFX |
...
| Lunge ghost | 30 | As ghost; brief 2.4× speed bursts every 60–140f; uses axis-separated collision during burst so it slides along walls | 15 | 10 |
...
| Ghoul | 50 | Slow crawl; leaps at 5.5× speed when in range; leap stops dead on wall contact | 40/contact | 35 |
| Long Ghoul | 100 | Like Ghoul but with 5 legs (one always longer), scrunched body, grey-white; leaps far more often (cooldown 20–55f); leap stops dead on wall contact | 40/contact | 70 |
...
| Boss (Ghoul) "ROTTEN PHILIP" | 438 | 3-phase leap boss; arrival: drops from top of screen, lands with screen shake + thud SFX; crawls toward player then leaps aggressively; leap stops dead on wall contact; phase 1: leap every 50–80f; phase 2: every 35–55f + ghoul minions; phase 3: every 20–35f + long ghoul minions; 2s invuln on phase change; floors 2,6,10… | 60/contact | 200 |
| Boss (Ashtaroth) "ASHTAROTH" | 500 | 4th boss cycle slot; bat-winged demon with horns; arrival: rises from flames at the bottom of the screen over 3s (ease-out quadratic); victory sequence triggers immediately on death; fires big slow homing meat lumps (invulnerable, leave gas trail) + phase 2+ fires circular barrage of small shootable lumps; phase 3: erratic movement; leaves long-lasting light-green gas trail that damages player (same colour family as Nuckelavee trail, slightly lighter: `COL_ASHTAROTH_GAS` #77ff99); 2s invuln on phase change; floors 4,8,12… | 35/contact + 30/big lump + 18/small lump + 10/trail | 200 |
...
| `iddqd` | Toggle god mode — player takes no damage; sprite turns gold |
...
| `runvictory` | Trigger the end-of-cycle victory animation immediately |
...
- **Boss death explosion** — multi-wave particle burst (5 waves with staggered delays) plus 8 scattered fragments. All bosses explode: `_spawnDeathFX` checks `enemy.type === 'boss' || enemy.type.endsWith('_boss')`, so any future `*_boss` type gets it automatically.
...
- **God mode** — `player.godMode` boolean (default `false`); toggled by the `iddqd` dev console command. Checked first in `player.takeDamage()` — all damage is silently ignored. Player sprite renders in gold (`#ffd700`) instead of `COL_PLAYER` while active. Persists across floors via `nextFloor()` (saved/restored alongside hp, powerups, etc.).
- **Dev console pause** — the game update loop in `draw()` checks `!G.devConsole.open` alongside `!G.mapOpen`; all game logic is frozen while the console is open (same pattern as the map).
- **Pause screen key guide** — `drawPaused()` renders a compact two-line key legend (move/aim/shoot + powerup/cycle/map) below the resume/quit prompts.
- **Leap wall collision** — Ghouls, LongGhouls, and GhoulBoss record position before `_resolveCollisions()` during a leap; if position changed (wall push occurred) the leap is cancelled and the cooldown timer reset. Regular crawl movement still bounces off walls normally.
- **Lunge ghost wall sliding** — during `this.lunging`, GhostEnemy uses axis-separated movement (move X → resolve, move Y → resolve) matching the player's collision style, so lunge ghosts slide along walls instead of bouncing.
...
- **Boss arrival animations** — Skull boss (`arriving` flag + `arriveTimer` 180f): spins with decelerating rotation while scaling from 0 to full size; plays `skull_boss_arrive` SFX (3s ominous coalescing: sub-bass sawtooth 38→72 Hz + eerie square tremolo 220→440 Hz slowing from 9→2 Hz + crash thud at 2.85s). Ghoul boss: drops from above screen with quadratic acceleration, screen shake on impact + `ghoul_boss_land` SFX. Ashtaroth: rises from flames at bottom of screen over 3s (ease-out quadratic from `C.HEIGHT + radius + 20` to `arriveTargetY`); 20 animated flame tongues rendered along the bottom edge during arrival (`COL_ASHTAROTH` outer + `#ffaa00` inner, sway with sin waves). All bosses invulnerable during arrival.
- **Ashtaroth boss** — `AshtarothBossEnemy` class; `COL_ASHTAROTH` (#ff4400) deep orange-red. Renderer: bat wings (curved splines with per-instance reach/droop variation via `deform[2–5]`), horns (per-instance tip offsets), glowing eye sockets (colour shifts phase 1→2→3), jagged maw (per-instance tooth heights). Body ellipse slightly offset and sized by `deform[0–1]`. Phase 1/2: pursues player; phase 3: erratic random-direction movement (new direction every 15–35f). Fires big meat lumps each `_lumpRate` (85/65/50f per phase), 1/2/3 lumps spread ±0.28 rad. Phase 2+ fires circular barrage of `_barrageCount` (8/12) small lumps every 290f. Gas trail (`ASHTAROTH_TRAIL_LIFETIME` 360f, `COL_ASHTAROTH_GAS` #77ff99 light green — same colour family as Nuckelavee trail) that deals 10 HP per 8f tick. `G.meatLumps[]` array in state (cleared on `enterRoom()` and boss death). On death: multi-wave explosion + victory sequence triggers immediately (no stairwell walk required); `checkRoomCleared` calls `cycleComplete()` directly after setting the stairwell direction.
...
- **Per-instance deformation (Mummy/Ashtaroth)** — `MummyEnemy`, `MummyBossEnemy`, and `AshtarothBossEnemy` each carry `deform[6]` (values −1..1, set at spawn). `_drawMummyBody()` uses them to jitter body/head quad corners (±1–1.5px × scale) and arm elbow/wrist positions; Ashtaroth uses them to vary wing reach/droop, horn tip position, body ellipse offset, and maw tooth heights (±5–10%).
